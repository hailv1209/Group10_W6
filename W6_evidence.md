...
| Cài Đặt | Giá Trị |
|---------|-------|
| **Loại Budget** | Lặp lại hàng ngày |
| **Giới Hạn Số Tiền** | USD $150 |
| **Ngưỡng Cảnh Báo** | 80% ($120 thực tế) |
| **Người Nhận Cảnh Báo** | SNS topic cho thông báo nhóm |

**Ảnh Chụp Bằng Chứng:**

```
[CHÈN ẢNH CHỤP: AWS Budgets console hiển thị daily recurring budget với giới hạn $150 và alert được cấu hình]
```
...
**Tên Function:** `w6-security-auto-remediate`  
**Ngôn Ngữ:** Python 3.11  
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:ec2:*:726411362669:security-group/*"
    },
    {
      "Effect": "Allow",
      "Action": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:sns:*:726411362669:w6-security-alerts"
    }
  ]
}
```
...
def lambda_handler(event, context):
    """
    Tự động remediate mở Security Groups: revoke 0.0.0.0/0 trên ports 22/3389
    Triggered bởi CloudTrail AuthorizeSecurityGroupIngress event hoặc daily cron
    """
    ...
    try:
        # Lấy tất cả security groups trong account
        ...
        for sg in sg_response['SecurityGroups']:
            ...
            # Kiểm tra từng ingress rule cho open SSH/RDP (0.0.0.0/0)
            for rule in sg.get('IpPermissions', []):
                from_port = rule.get('FromPort')
                to_port = rule.get('ToPort')
                
                # Target ports: SSH (22) và RDP (3389)
                vulnerable_ports = [22, 3389]
                
                if from_port in vulnerable_ports or to_port in vulnerable_ports:
                    # Kiểm tra nếu có rule cho phép 0.0.0.0/0
                    for ip_range in rule.get('IpRanges', []):
                        if ip_range.get('CidrIp') == '0.0.0.0/0':
                            ...
                            try:
                                ...
                                print(f"Đã revoke 0.0.0.0/0 trên port {from_port} từ SG {sg_id} ({sg_name})")
                                ...
                                print(f"Lỗi revoke rule trên {sg_id}: {str(e)}")
    
    except Exception as e:
        print(f"Lỗi fatal trong remediation: {str(e)}")
        return {
            'statusCode': 500,
    ...
    if remediated:
        sns.publish(
            ...
            Subject='W6 Security Guard: Auto-Remediation Summary',
